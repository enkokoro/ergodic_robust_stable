...
class Agent:
    def __init__(self, agent_id, init_pos, max_control, k_bands, U_shape, fourier_functions, eps=1e-5): 
        ...
        e_init = 0
        for k in k_bands:
            e_init += self.ff[k]['lambda_k']*(self.ff[k]['f_k'](init_pos)-self.ff[k]['mu_k'])**2
        self.e_log = [e_init] 
...
class AgentSystem:
    def __init__(self, agents, init_mu, U_shape, fourier_functions, K): 
        ...
        self.c_k_log = [self.c_k]
        self.e_log = [calculate_ergodicity(np.ndindex(*[self.K]*self.n), self.c_k, self.ff)]

    def calculate_c_k(self):
        c_k = {}
        for k in np.ndindex(*[self.K]*self.n):
    ...
    def evolve(self, t, delta_t, u_agents=None, c_k_agents_prev=None, x_agents_prev=None):
        ...
        self.c_k = self.calculate_c_k()
        ergodicity_agents = calculate_ergodicity(np.ndindex(*[self.K]*self.n), self.c_k, self.ff)
        if not prediction_mode:
